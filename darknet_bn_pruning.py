...
class DarknetBnPruning(nn.Module):
    def __init__(self, cfgfile):
        super(DarknetBnPruning, self).__init__()
    ...
    def forward(self, x, y = None):
        ...
        for block in self.blocks:
            ...
            if block['type'] == 'net':
                ...
                # Save conv weights for forward of bnx layers
                conv_weight = None
                if block['type'] == 'convolutional' and ind <= 95:
                        ...
                        if module_name.startswith('conv'):
                            for param_name, param in module.named_parameters():
                                if param_name == 'weight':
                                    conv_weight = param
                            x = module(x)
                        elif module_name.startswith('bnx'):
                            x = module(x, conv_weight)
                        else:
                            x = module(x)
                else:
                    x = self.models[ind](x)
            ...
            if block['type'] == 'net':
                ...
                if block['type'] == 'convolutional':
                    ...
                    if batch_normalize:
                        ...
                        # Only segmentation layer and omit the very beginning and the last layer
                        if ind <= 95:
                            model.add_module('bnx{0}'.format(conv_id), bn.BatchNorm2dEx(filters))
                        else:
