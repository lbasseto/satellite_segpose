...
def update_pose_according_to_new_mesh(flist, outimgdir, outposedir, meshTrans):
    if not os.path.exists(outimgdir):
        os.makedirs(outimgdir)  # recursive
    if not os.path.exists(outposedir):
        os.makedirs(outposedir)  # recursive
    for imgpath in flist:
        ...
        shutil.copyfile(imgpath, outimgdir + '/' + baseName + extName)
        ...
        outposefilename = outposedir + '/' + baseName + '.txt'
        np.savetxt(outposefilename, outpose)
        print(outposefilename)

def refine_mesh_center(linemod_path, outmodelpath, outimgpath, outposepath, listfile):
    ...
    for objname in objects:
        ...
        # refine the raw pose
        update_pose_according_to_new_mesh(fileset[objname], outimgpath + objname, outposepath + objname, meshTrans)
...
if __name__ == "__main__":
    ...
    outposepath = '/home/yhu/data/LINEMOD/mask/'
    listfile = '/home/yhu/data/LINEMOD/raw_all.txt'
    refine_mesh_center(linemod_path, outmodelpath, outimgpath, outposepath, listfile)
