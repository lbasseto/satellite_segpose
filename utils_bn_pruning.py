...
from darknet_bn_pruning import EmptyModule
...
def count_sparse_bn(model, writer, epoch, w, h):
    total = 0.
    ...
    for l1, l2 in zip(ls, ls[1:]):
        if isinstance(l1, nn.Conv2d) and isinstance(l2, (BatchNorm2dEx, nn.BatchNorm2d)):
...
def compress_convs(model, compressed, cfgfile):
    ...
    new_model = compressed(cfgfile, channels)
    ...
    for original, compressed in zip(expand_model(model, []), expand_model(new_model, [])):
        ...
        print("===\n\n\n\n")
        if not isinstance(original, (nn.Sequential, nn.MaxPool2d, nn.LeakyReLU, EmptyModule, PoseSegLayer, Pose2DLayer)):
